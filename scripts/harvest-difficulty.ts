 ...
 *
 *   npm run gen:difficulty -- --players 600
 */
import { readFile, writeFile, mkdir } from "node:fs/promises";
import { gunzipSync } from "node:zlib";
import { readNbt, bagItemsFrom } from "../src/lib/nbt";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";

/**
 * Wrapped in a function rather than run at the top level, so this file can be TypeScript: tsx
 * compiles a top-level .ts as CommonJS, where top-level await is a syntax error. TypeScript is
 * what lets it share the NBT reader with the app instead of carrying a second copy of it.
 *
 * It is TypeScript for the module resolution alone, not for the types — scripts/ is outside the
 * typecheck, the same as every other build script here, all of which are .mjs.
 */
async function main(): Promise<void> {
  ...
  function completedIds(member) {
    ...
    // Accessories, so the ones nobody can buy can be ranked by how many players have one. They
    // are the only grind whose completion is not a flag on the profile but an item in a gzipped
    // blob, which is why they were the one grind with no difficulty behind them.
    const bag = member.inventory?.bag_contents?.talisman_bag?.data;
    if (bag) {
      try {
        const root = readNbt(new Uint8Array(gunzipSync(Buffer.from(bag, "base64"))));
        for (const item of bagItemsFrom(root)) ids.add(`accessory_${item.id}`);
      } catch {
        // An unreadable bag is one player's missing data, not a reason to lose the other 599.
      }
    }
